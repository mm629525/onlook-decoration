...
connection.prototype.joinPublicGroup = function(options) {
  var roomJid =
    this.context.appKey + '_' + options.roomId + '@conference.' + this.domain
...
connection.prototype.isOpening = function() {
  var status = this.context.status
  return (
    status == _code.STATUS_DOLOGIN_USERGRID || status == _code.STATUS_DOLOGIN_IM
  )
}
...
connection.prototype.clear = function() {
  ...
  if (
    this.errorType == WebIM.statusCode.WEBIM_CONNCTION_CLIENT_LOGOUT ||
    this.errorType == -1
  ) {
  }
}
...
connection.prototype._onLeaveGroup = function(info) {
  ...
  var options = {
    ...
    msg:
      'You have been out of the group:' + info.group_id + '.Reason:' + info.msg,
    agree: function agree() {}
  }
  this.onConfirmPop(options)
}
...
connection.prototype._onReceiveRejectionFromGroup = function() {
  ...
  var options = {
    title: 'Group notification',
    msg:
      'You have been rejected to join into the group:' + info.group_name + '.',
    agree: function agree() {}
  }
  this.onConfirmPop(options)
}
